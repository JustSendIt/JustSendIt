...
(function () {
  ...
  function renderHero(c) {
    ...
    const optBtn = requalify
      ...
      : '<button class="btn ' + (joined ? 'btn-ghost' : 'btn-primary') + ' comm-join-btn" id="comm-join" type="button" data-tip="Opts you in, or a second tap leaves and wipes your level" aria-pressed="' + (!!joined) + '">' + (joined ? '✓ You’re in' + (c.mine && c.mine.isCreator ? ' (starter 👑)' : '') : '➕ Opt in' + (live ? ' & get 10×' : '')) + '</button>';
    ...
    heroEl.innerHTML = banner +
      '<div class="comm-hero-body">' +
        ...
        '<details class="grules comm-rules"><summary>📖 How points, levels &amp; the 10× work</summary><div class="grules-body"><ul class="comm-rules-list">' +
          ...
          '<li>💎 <b>Your conviction</b> here rises <b>only when you do something</b> — join, post, comment, react, call the token, vote on a proposal. Never for time alone, and only from a verified holder slot' + (c.demo ? ' — a member level for this page and its members list; the sandbox has no token to pin on your wall.' : '; it shows next to $' + esc(c.symbol) + ' in your public wall’s <b>Conviction Plays</b> section.') + '</li>' +
          '<li>👑 The starter earns a one-time <b>founder bonus</b> when the community reaches ' + c.goLive.need + ' <b>verified holder slots</b> — wallets that really hold the token, not taps of Join.</li>' +
        '</ul></div></details>' +
      '</div>';
